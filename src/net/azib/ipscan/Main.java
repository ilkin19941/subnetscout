...
import net.azib.ipscan.gui.FirstRunLanguageDialog;
...
public class Main {
	...
	public static void main(String... args) {
		...
		try {
			...
			if (!config.firstRunDone && args.length == 0) {
				config.language = FirstRunLanguageDialog.open();
				config.firstRunDone = true;
				config.store(); // сохраняем сразу, чтобы выбор не потерялся при сбое
			}
